...
- **可定制的桌面外观**：在通用设置中选择兼容或云母玻璃效果，并可使用本地壁纸、
  调整模糊、磨砂和背景亮度；设置同时覆盖侧边栏与对话区域。
- **开箱即用的发布版**：安装包内置与当前平台匹配的 Node 运行时和 Host 依赖，终端
  用户无需额外安装 Node.js 或 pnpm。
- **保留源码开发体验**：支持从源码安装依赖、启动调试环境、运行测试和原生平台打包。
- **三端交付**：发布流程构建 Windows x64、macOS Apple Silicon 和 Linux x64
  产物。
...
`pnpm dev` 会依次构建桌面连接层、Host、透明外观插件与官方渲染器所需插件，并启动
Electron 开发环境。
