 ...
 * - every glass knob into the General settings page's right content pane
...
import { restoreDesktopWallpaper } from './desktop-wallpaper.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  void restoreDesktopWallpaper(window.desktopAppearance, layer, localStorage.getItem('desktop-appearance'))

  // Two store mirrors of the same layer state: one for the Plugins card
  // (master switch) and one for the General page's right-pane controls.
  ...
  // both stores so the General page controls re-render with the new range.
  ...
  // The General section projects this slot into the modal's right content
  // pane. `settings.section` would add a separate left-nav entry instead.
