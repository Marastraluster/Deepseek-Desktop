...
/**
 * Desktop's upstream-derived compatibility layer. Unlike the upstream Aqua
 * preset, this leaves brand, typography, and interaction colors to Harness
 * and changes only visual surfaces needed for wallpaper translucency.
 */
const DESKTOP_COMPAT_TOKEN_OVERRIDES: ThemeTokenOverrides = {
  '--dsw-alias-bg-base': { light: 'transparent', dark: 'transparent' },
  '--dsw-alias-bg-layer-1': { light: 'rgba(255, 255, 255, 0.50)', dark: 'rgba(18, 21, 28, 0.54)' },
  '--dsw-alias-bg-layer-2': { light: 'rgba(255, 255, 255, 0.56)', dark: 'rgba(18, 21, 28, 0.60)' },
  '--dsw-alias-bg-layer-3': { light: 'rgba(255, 255, 255, 0.62)', dark: 'rgba(18, 21, 28, 0.66)' },
  '--dsw-alias-bg-overlay': { light: 'rgba(255, 255, 255, 0.68)', dark: 'rgba(18, 21, 28, 0.72)' },
  '--dsw-alias-bg-module-platform': { light: 'rgba(255, 255, 255, 0.52)', dark: 'rgba(18, 21, 28, 0.56)' },
  '--dsw-alias-bg-multi-select': { light: 'rgba(255, 255, 255, 0.60)', dark: 'rgba(18, 21, 28, 0.64)' },
  '--dsw-specific-sidebar-fill': { light: 'rgba(255, 255, 255, 0.34)', dark: 'rgba(16, 19, 25, 0.40)' },
  '--dsw-specific-menu': { light: 'rgba(255, 255, 255, 0.66)', dark: 'rgba(18, 21, 28, 0.70)' },
  '--dsw-specific-selector': { light: 'rgba(255, 255, 255, 0.62)', dark: 'rgba(18, 21, 28, 0.66)' },
  '--dsw-specific-bubble': { light: 'rgba(255, 255, 255, 0.46)', dark: 'rgba(18, 21, 28, 0.50)' },
  '--dsw-specific-bubble-highlight': { light: 'rgba(255, 255, 255, 0.56)', dark: 'rgba(18, 21, 28, 0.60)' },
  '--dsw-specific-input-major': { light: 'rgba(255, 255, 255, 0.64)', dark: 'rgba(18, 21, 28, 0.68)' },
  '--dsw-specific-login-input': { light: 'rgba(255, 255, 255, 0.64)', dark: 'rgba(18, 21, 28, 0.68)' },
  '--dsw-alias-markdown-code-block': { light: 'rgba(255, 255, 255, 0.48)', dark: 'rgba(16, 19, 25, 0.56)' },
  '--dsw-alias-markdown-code-block-banner': { light: 'rgba(255, 255, 255, 0.56)', dark: 'rgba(16, 19, 25, 0.62)' },
  '--dsw-alias-markdown-inline-code': { light: 'rgba(255, 255, 255, 0.48)', dark: 'rgba(16, 19, 25, 0.56)' },
}

const DESKTOP_MICA_TOKEN_OVERRIDES: ThemeTokenOverrides = {
  // Mica surfaces are rendered by the dedicated CSS layer. The frame and
  // sidebar fill must stay clear so the Electron wallpaper remains visible.
  '--dsw-alias-bg-base': { light: 'transparent', dark: 'transparent' },
  '--dsw-specific-sidebar-fill': { light: 'transparent', dark: 'transparent' },
}

export function selectAquaTokenOverrides(mode: AquaSettings['mode']): ThemeTokenOverrides {
  return mode === 'mica' ? DESKTOP_MICA_TOKEN_OVERRIDES : DESKTOP_COMPAT_TOKEN_OVERRIDES
}
...
function readMode(): 'mica' | 'compat' {
  try {
    return localStorage.getItem(MODE_KEY) === 'mica' ? 'mica' : 'compat'
  } catch {
    return SETTINGS_DEFAULTS.mode
  }
}
...
export class AquaLayer {
  ...
  private applyTokens(): void {
    ...
    this.tokenDisposer = this.ctx.theme.overrideTokens(
      OVERRIDE_SOURCE,
      selectAquaTokenOverrides(this.settings.mode),
    )
  }
